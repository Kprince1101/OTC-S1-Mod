...
namespace OverTheCounter.Apps
{
    public partial class GreenTabApp
    {
        private void RefreshFloorCards()
        {
            ...
            var styles = FloorStyle.All.Values.ToList();
            ...
            for (int i = 0; i < styles.Count; i++)
            {
                ...
                bool isEquipped = style.Id == currentFloor;
                bool isSelected = style.Id == _pendingFloorId;
                bool canAfford = CanAffordFloor(currentFloor, style.Id, balance);

                float xPos = 10 + col * (CARD_WIDTH + CARD_GAP);
                float yPos = -(8 + row * (CARD_HEIGHT + CARD_GAP));

                var card = CreateFloorCard(style, isEquipped, isSelected, canAfford, xPos, yPos);
                _cardEntries.Add(card);
            }
        }

        private CardEntry CreateFloorCard(FloorStyle style, bool isEquipped, bool isSelected, bool canAfford,
            float xPos, float yPos)
        {
            ...
            var swatchColor = FloorSwatchColors.TryGetValue(style.Id, out var sc) ? sc : TextMuted;
        ...
        private void ApplyFloorUpgrade()
        {
            ...
            float balance = GetOnlineBalance();
            float cost = GetFloorUpgradeCost(currentFloor, _pendingFloorId);
            ...
            if (cost > 0)
            {
                try
                {
                    ...
                    mm?.CreateOnlineTransaction("Floor Upgrade", -cost, 1, $"Upgraded to {newStyle.DisplayName}");
                }
                catch (Exception ex)
                {
                    OTCLog.Error(OTCLog.Systems.Patch, $"Floor payment failed: {ex.Message}");
                    return;
                }
            }
        ...
        private static float GetFloorUpgradeCost(string fromId, string toId)
        {
            var from = FloorStyle.Get(fromId);
            var to = FloorStyle.Get(toId);
            float diff = to.Cost - from.Cost;
            return diff > 0 ? diff : 0;
        }

        private static bool CanAffordFloor(string currentId, string targetId, float balance)
        {
            if (currentId == targetId) return true;
            float cost = GetFloorUpgradeCost(currentId, targetId);
